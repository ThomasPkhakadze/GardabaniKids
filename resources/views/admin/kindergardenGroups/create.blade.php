...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.kindergardenGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.kindergarden-groups.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vacancy') ? 'is-invalid' : '' }}" type="number" name="vacancy" id="vacancy" value="{{ old('vacancy', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kindergarden_branch') ? 'is-invalid' : '' }}" name="kindergarden_branch_id" id="kindergarden_branch_id" required>
                    @foreach($kindergarden_branches as $id => $entry)
                        <option value="{{ $id }}" {{ old('kindergarden_branch_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
